...
- (IBAction)moveSite {
    ...
    NSString *urlString = [NSString stringWithFormat:@"https://%@/reader/move_feed_to_folder",
                           NEWSBLUR_URL];
...
- (IBAction)moveFolder {
    ...
    [self.errorLabel setHidden:YES];
    [self.activityIndicator startAnimating];
    NSString *urlString = [NSString stringWithFormat:@"https://%@/reader/move_folder_to_folder",
                           NEWSBLUR_URL];
    NSURL *url = [NSURL URLWithString:urlString];
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
