...
- (void)loadFriendsList:(NSString *)query {    
    NSString *urlString = [NSString stringWithFormat:@"https://%@/social/find_friends?query=%@&limit=10",
                           NEWSBLUR_URL,
                           query];
...
- (void)loadSuggestedFriendsList {
    NSString *urlString = [NSString stringWithFormat:@"https://%@/social/load_user_friends",
                           NEWSBLUR_URL];
    NSURL *url = [NSURL URLWithString:urlString];
