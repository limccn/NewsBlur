...
- (void)fetchFeedDetail:(int)page withCallback:(void(^)())callback {
    ...
    if (callback || (!self.pageFetching && !self.pageFinished)) {
        ...
        if (appDelegate.isSocialView) {
            theFeedDetailURL = [NSString stringWithFormat:@"https://%@/social/stories/%@/?page=%d", 
                                NEWSBLUR_URL,
                                [appDelegate.activeFeed objectForKey:@"user_id"],
                                self.feedPage];
        } else {
            theFeedDetailURL = [NSString stringWithFormat:@"https://%@/reader/feed/%@/?page=%d", 
                                NEWSBLUR_URL,
        ...
        if (appDelegate.isSocialRiverView) {
            if ([appDelegate.activeFolder isEqualToString:@"river_global"]) {
                theFeedDetailURL = [NSString stringWithFormat:
                                    @"https://%@/social/river_stories/?global_feed=true&page=%d",
                                    NEWSBLUR_URL,
                                    self.feedPage];
                
            } else {
                theFeedDetailURL = [NSString stringWithFormat:
                                    @"https://%@/social/river_stories/?page=%d", 
                                    NEWSBLUR_URL,
                                    self.feedPage];
            }
        } else if ([appDelegate.activeFolder isEqual:@"saved_stories"]) {
            theFeedDetailURL = [NSString stringWithFormat:
                                @"https://%@/reader/starred_stories/?page=%d",
                                NEWSBLUR_URL,
                                self.feedPage];
        } else {
            theFeedDetailURL = [NSString stringWithFormat:
                                @"https://%@/reader/river_stories/?feeds=%@&page=%d", 
                                NEWSBLUR_URL,
...
- (void)markFeedsReadWithAllStories:(BOOL)includeHidden {
    if (appDelegate.isRiverView && includeHidden &&
        ...
        // Mark folder as read
        NSString *urlString = [NSString stringWithFormat:@"https://%@/reader/mark_all_as_read",
                               NEWSBLUR_URL];
        ...
        // Mark folder as read
        NSString *urlString = [NSString stringWithFormat:@"https://%@/reader/mark_feed_as_read",
                               NEWSBLUR_URL];
        NSURL *url = [NSURL URLWithString:urlString];
        ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
        ...
        NSString *urlString = [NSString stringWithFormat:@"https://%@/reader/mark_feed_stories_as_read",
                               NEWSBLUR_URL];
        NSURL *url = [NSURL URLWithString:urlString];
        ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
...
- (void)deleteSite {
    ...
    NSString *theFeedDetailURL = [NSString stringWithFormat:@"https://%@/reader/delete_feed", 
                                  NEWSBLUR_URL];
...
- (void)deleteFolder {
    ...
    NSString *theFeedDetailURL = [NSString stringWithFormat:@"https://%@/reader/delete_folder", 
                                  NEWSBLUR_URL];
    NSURL *urlFeedDetail = [NSURL URLWithString:theFeedDetailURL];
...
- (void)instafetchFeed {
    NSString *urlString = [NSString
                           stringWithFormat:@"https://%@/reader/refresh_feed/%@", 
                           NEWSBLUR_URL,
                           [appDelegate.activeFeed objectForKey:@"id"]];
    [self cancelRequests];
