...
- (IBAction)tapNextButton {
    ...
    if (self.selectedCategories_.count) {
        NSString *urlString = [NSString stringWithFormat:@"https://%@/categories/subscribe",
                               NEWSBLUR_URL];
...
- (void)importFromGoogleReader {    
    ...
    NSString *urlString = [NSString stringWithFormat:@"https://%@/import/import_from_google_reader/",
                           NEWSBLUR_URL];
    NSURL *url = [NSURL URLWithString:urlString];
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
...
- (void)addSite:(NSString *)siteUrl {
    NSString *urlString = [NSString stringWithFormat:@"https://%@/reader/add_url",
                           NEWSBLUR_URL];
    NSURL *url = [NSURL URLWithString:urlString];
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
